...
## 重构
随着程序的演化，代码也需要跟着演化。重写、重做和重新架构代码合起来，成为重构（refactoring）。不要对改动犹豫不决，应该现在就做，当代码出现下面的任意特征时，你都应该考虑重构代码：
* **重复。**违反了DRY原则。
* **非正交的设计。**有些代码或设计可以变得更为正交。
* **过时的知识。**事情变了、需求转移了、或者有了新的理解。
* **性能。**迁移功能以改善性能。

**Refactor Early, Refactor Often——早重构，常重构。**
时间压力常常被用作不能进行重构的借口，然而，如果现在不做，沿途修正问题将需要投入更多的时间。如果你不能立刻进行重构，就一定要把它列入计划。
重构是一项需要慎重、深思熟虑，小心进行的活动，其核心是重新设计，无节制的修改会让事情变得更加糟糕。以下是重构的一些提示：
1. 不要试图在重构的同时增加功能。
2. 在开始重构之前，确保你拥有良好的测试。当改动破坏了任何东西，你能及时发现。
3. 采取短小、深思熟虑的步骤。进行许多局部改动，继而产生更大规模的改动。
